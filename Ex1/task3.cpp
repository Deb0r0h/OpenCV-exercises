...
void max(Mat image,Mat final_image,int kernel_size)
{
    if(kernel_size%2 == 0)
    {
        cout<<"Kernel size not valid, only odd number"<<endl;
         throw "ERROR";
    }
...
int main(int argc, char** argv)
{
	...
	Mat img_gray, final_imageMIN, final_imageMAX,final_imageMEDIAN, final_imageGAUSSIAN;
	cvtColor(img,img_gray,COLOR_RGB2GRAY);
	cvtColor(img,final_imageMIN,COLOR_RGB2GRAY);
	cvtColor(img,final_imageMAX,COLOR_RGB2GRAY);
    cvtColor(img,final_imageMEDIAN,COLOR_RGB2GRAY);
    cvtColor(img,final_imageGAUSSIAN,COLOR_RGB2GRAY);


    max(img_gray,final_imageMAX,3);
	min(img_gray,final_imageMIN,3);
	medianBlur(img_gray,final_imageMEDIAN,7);
    GaussianBlur(img_gray,final_imageGAUSSIAN,Size(7,7),0.5); //dev std 0.5
	...
	imshow("Modified image MAX", final_imageMAX);
     //MEDIAN
    namedWindow("Modified image MEDIAN");
	imshow("Modified image MEDIAN", final_imageMEDIAN);
	imwrite("MEDIAN7.jpg", final_imageMEDIAN);
    //GAUSSIAN
    namedWindow("Modified image GAUSSIAN");
	imshow("Modified image GAUSSIAN", final_imageGAUSSIAN);
	imwrite("GAUSSIAN7.jpg", final_imageGAUSSIAN);
