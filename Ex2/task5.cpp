...
void onMouse(int event,int x,int y,int f, void* userdata)
{
    if(event == EVENT_LBUTTONDOWN)
    {
        ...
        int T = 80; // cambiandolo riesco a selezionare solo la palla
        ...
        Mat colorMask = image.clone();
        Scalar colore = Scalar(255,5,40);
        ...
        for(int i=0;i<colorMask.rows;i++)
        {
            for(int j=0;j<colorMask.cols;j++)
            {
                ...
                {
                    colorMask.at<Vec3b>(i,j)[0]  = colore[0];
                    colorMask.at<Vec3b>(i,j)[1]  = colore[1];
                    colorMask.at<Vec3b>(i,j)[2]  = colore[2];
                }
            }
        }
